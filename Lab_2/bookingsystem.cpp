...
BookingSystem::BookingSystem(const BookingSystem &other)
{
    ...
    for(int i = 0; i < numPerson; i++){
        if((maxNumEvent - i) > numEvent){
            events[maxNumEvent-1-i] = new Event(*(other.persons[i]->event));
            
        }
        if((maxNumTicket - i) > numTicket){
            tickets[maxNumTicket-1-i] = new Ticket(*(other.persons[i]->ticket)); 
        }
        
        persons[i] = new Person(*(other.persons[i]), other.persons[i]->event, other.persons[i]->ticket);
    }
    ...
    for(int i = 0; i < numPerson; i++){
        if((maxNumEvent - i) > numEvent){
            if(numEvent == 0){
               ...
               delete events[maxNumEvent-1-i]; 
            }
            persons[i]->event = nullptr;
        }
        ...
        if((maxNumTicket - i) > numTicket){
            if(numTicket == 0){
               ...
               delete tickets[maxNumTicket-1-i]; 
            }
            persons[i]->ticket = nullptr;
        }    
